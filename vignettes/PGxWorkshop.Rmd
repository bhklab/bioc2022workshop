...
* `r BiocStyle::CRANpkg("data.table")``
...
```{r SummarizedExperiment_schema, echo=FALSE, fig.small=TRUE, fig.cap=" **SummarizedExperiment**"}
knitr::include_graphics("./SummarizedExperiment_schematic.svg")
```

```{r MultiAssayExperiment_schema, echo=FALSE, fig.small=TRUE, fig.cap=" **MultiAssayExperiment**"}
...
For this workshop we have included a subset of the NCI_ALAMANAC_2017 
`PharmacoSet`, curated from [@holbeck_national_2017]
...
data(NCI_ALMANAC_raw)
knitr::kable(head(NCI_ALMANAC_raw))
```
...
groups1 <- list(
  rowDataMap=c("treatment1id", "treatment2id", "treatment1dose", "treatment2dose"),
...
str(rawdata(treDataMapper))
```

```{r data_mapper_correct_guess, eval=TRUE, echo=TRUE}
groups2 <- list(
  rowDataMap=c("treatment1id", "treatment2id", "treatment1dose", "treatment2dose", "replicate"),
  colDataMap=c("sampleid"),
  assayMap=c(
    "treatment1id", "treatment2id", "treatment1dose", "treatment2dose", "replicate",
    "sampleid"
  )
)
...
```{r updating_the_data_mapper, eva=TRUE, echo=TRUE}
...
(NCI_ALAMANAC_2017 <- PharmacoSet2(
...
NCI.TRE <- treatmentResponse(NCI_ALAMANAC_2017)

## TreatmentResponseExperiment method
NCI.TRE |>
    subset(is.na(treatment2dose)) -> NCI.mono
...
NCI.mono |> 
  ...
  assay("sensitivity", withDimnames=TRUE) |> 
  aggregate2(
    by=c("treatment1id", "sampleid", "replicate"),
    {
...
As you can see, by using the `aggregate2` method, we are able to 
...
```{r}
NCI.mono$Vincristine.profiles <- Vincristine.monotherapy
NCI.mono$Vincristine.profiles[order(sampleid)]
```
...
```{r}
treatmentResponse(NCI_ALAMANAC_2017) |> subset(treatment2id=="") |> aggregate(assay="sensitivity", N = .N, by=c("treatment1id", "treatment1dose", "sampleid")) |> with(expr=hist(N))
...
mean_bliss_score <- combo_profiles2[, .(Mean_Bliss = mean(Bliss_score)), .(treatment1id, treatment2id, sampleid)]
...
synergy_summaries <- reshape2::dcast(mean_bliss_score, treatment1id + treatment2id ~ sampleid)

rownames(synergy_summaries) <- paste(synergy_summaries$treatment1id, synergy_summaries$treatment2id, sep="_+_")
...
gene.exp.sigs <- drugSensitivitySig(NCI_ALAMANAC_2017, 
