...
four `RangedSummarizedExperiment` objects. The molecular data types are:
  ...
  3. "rnaseq.comp" contains RNA sequencing dat.
...
NCI_tre <- treatmentResponse(NCI_ALAMANAC_2017)
...
```{r}
treatmentResponse(NCI_ALAMANAC_2017) |>
  subset(treatment2id == "") |>
  aggregate(
      assay="sensitivity",
      N=.N,
...
combo_profiles <- treatmentResponse(nci)$sensitivity |>
  subset(treatment2id != "")
...
```{r}
gene_exp_sigs <- drugSensitivitySig(NCI_ALAMANAC_2017,
