...
We can then apply the aggregate function function to fit the curves to the mean viability. Here, we aggregate while setting the `by` argument to compute
an aggregation over each unique combination of `treatment1id`, `sampleid` and `bio_rep`.
...
We can now finally compute the reference combination viabilites. Again, we can use aggregate to make the calculation happen for each sample,
