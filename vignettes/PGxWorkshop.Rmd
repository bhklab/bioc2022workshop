...
```{r extracting_monotherapy_from_tre, warning=FALSE}
...
```{r fitting_hill_curve_to_tre, warning=FALSE}
NCI_mono |>
...
As you can see, by using the `aggregate` method, we are able to to fit our
the 3-parameter Hill curve to our data, then use the fit parameters to
simultaneously calculate AUC and IC50. This is the power of using the
`aggregate` method with `enlist=FALSE`.

Once we have values computed in a `data.table`, we can store them back in the
`TreatmentResponseExperiment`. Notice that the TRE object handles seemlessly
the fact that we have values computed for only a subset of experiments.

```{r vincristine_assignment, warning=FALSE}
...
Note that by default, `logLogisticRegression` returns the `E_inf` parameter on a percentage scale. We convert to a fraction scale because 
that makes the subsequent calculations simpler.  
...
Finally, we can assign the `combo_response` matrix back to our `TRE` seamlessly:

```{r assigning_combo_responses}

treatmentResponse(nci)$combo_response <- combo_response
...
```{r compute_combo_references, warning=FALSE}
...
Mathmatically, ZIP defines two values:
...
By default, `logLogisticRegression` returns parameters with E_inf on a
percentage scale. Here we convert that to fraction to simplify the formulas
for calculating expected combination viabilities.

```{r fix_e_inf, warning=FALSE}
...
We then make predictions for each of the measured combination doses using fit
corresponding parameters and the Hill Curve models.
...
We can now finally compute the reference combination viabilites. Again, we can
use `aggregate` to make the calculation happen for each sample,
pair of treatments, pair of doses, and replicate.
...
Note that while we wrote out all the functions above, we could also use built
in PharmacoGx functions for computing these scores, as below:

```{r, compute_combo_references_pgx, eval=FALSE, warning=FALSE}
combo_response |>
  aggregate(
...
Once we have the reference/expected viabilities under each Null Model assumption,
we can compute the related Combination Index or Score.
