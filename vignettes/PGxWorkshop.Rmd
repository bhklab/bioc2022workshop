...
<!-- todo: add attribution for the core BioC figures -->
...
* Petr Smirnov <petr.smirnov@embl.de>


Co-Authors of Workshop Materials and Packages:
...
focus of this workshop. Additional infrastructure available in this suite of packages includes
...
on storing, analyzing and visualizing drug
...
| Activity                                                   | Time |
|------------------------------------------------------------|------|
| Introduction to the workshop and learning goals            |  5m  |
| Overview of Drug Combination Experiments                   |  10m |
| Introduction to Bioconductor Project                       |  5m  |
| Overview of Bioconductor Data Structures                   |  10m |
| Hands on with a `MultiAssayExperiment`                     |  5m  |
| Introduction to CoreGx and PharmacoGx                      |  5m  |
| Introduction to the `CoreGx` data structures               |  10m |
|------------------------------------------------------------|------|
| How the `TRE` Supports Drug Combinations Data Analysis     |  10m |
| Using a `DataMapper` to build a Drug Combo PharmacoSet     |  20m |
| Computing with a `TRE`                                     |  15m |
|------------------------------------------------------------|------|
| Dose Response Models and Drug Sensitivity measures         |  10m |
| Drug Combination Synergy Models                            |  25m |
...
suppressPackageStartupMessages({
    library(pgxDrugCombinationWorkshop)
...
Bioconductor is an open source, community driven software project and which 
provides a set of data structures and tools for the analysis of biological data 
in R. The project started circa 2002 and has grown to include 2230+ software
packages, both developed by the core Bioconductor team (a minority), as well as 
many external contributors. In general, these packages include Software 
(Tools and Algorithms for analysis or visualization), Annotation Data packages, 
Experimental Data packages, and Workflows giving examples of particular analysis 
tasks. Inclusion of a package in Bioconductor requires it to conform to a set of 
standards regarding code quality, documentation, and interoperability with other 
packages in the ecosystem, the passing of an extended series of automated checks,
as well as a review by a member of the core team.


As mentioned above, one of the major goals of the Bioconductor project is 
to provide an interoperable set of data structures in which to store biological data,
which should accelerate the ability of users to apply novel methods and packages 
to their data. As any open source project, this interoperability is not universal,
but most Bioconductor pacakges will either use standard Bioconductor data types, or 
provide methods to convert to and from them. This means that learning the core 
Bioconductor classes, and the common abstractions between them opens up 100s of packages
for use in your analysis.


For the purposes of this workshop, we will focus on data structures that are used to store
molecular readouts from human samples, and an extention of these data structures to store 
complex phenotypic measurements, including drug combination screens, conducted on the 
same model systems. 
...
The `SummarizedExperiment` (`SE`) is one of the basic data containers in the
Bioconductor ecosystem. This data class was developed to store biological 
data that can be abstracted as measurements of a set of the same features across
multiple samples. Several `assays` can be stored in the same object, with the restriction
that they should map to the same set of features and sample identifiers, that is, 
that they are congruent. This is often useful to store measurements of features at 
different steps of processing. For example, a `SE` can store the raw counts as well 
as normalized RNA expression for the same genes on the same samples.


These data are stored together with a set of column and row annotations that describe the
features and samples. These annotations are stored in the `colData` and `rowData` slots of the
`SE` object. The rowData can also be defined as a `GenomicRanges` object, which allows you to 
explicitly identify features with genomic coordinates, useful for storing measurements derived 
from the sequencing of DNA or RNA (e.g., mutations, copy number abberations, CHIP-seq peaks, etc).
...
The `MultiAssayExperiment` (`MAE`) is an extension of the `SummarizedExperiment`
class that allows for the storage of multiple assays that are not necessarily
congruent. This is useful for storing the results of diverse multi-omic 
profiling experiments, including mutation, RNA expression, copy number abberation 
and protein abundace. 


```{r MAE_schema, echo=FALSE, fig.wide=TRUE, fig.cap = "**SummarizedExperiment** and **MultiAssayExperiment** classes."}
knitr::include_graphics("./MultiAssayExperiment_schematic.png")

```


At the heart of both objects are feature by sample matrices, with the major
difference being that the `SE` require congruent matrices (i.e., observations
on the same set of features and samples) where as the `MAE` allows storage of
disjoint sets of feature and samples via the use of the `sampleMap`, which,
conceptually, stores a graph of the mapping from samples to each assay in the
object.


### Hands on with a MultiAssayExperiment

For the majority of this workshop, we will be working with the NCI-ALMANAC,
a large-scale drug combination screen conducted by the National Cancer Institute.

The NCI-ALMANAC study did not conduct its own set of molecular profiling
experiments, since the cell lines screened are part of the NCI60 collection. 
With this workshop, we included some of the NCI60 molecular datasets as a
`MultiAssayExperiment` object.


Lets examine the included molecular data:
...
3 `RangedSummarizedExperiment` objects. The molecular data types are:
...
To gain familiarity with working with `MultiAssayExperiment` objects, we will
try to answer 3 questions:

1. What is the most represented tissue type in the profiled NCI60 cell lines?
2. What is the cell line expressing the highest expression of the MYC oncogene?
3. What is the tissue type with the highest expression of HER2?



# Introduction to CoreGx and PharmacoGx

The `r PGx` package was developed to provide a set of data structures and 
tools to work with the results of high-throughput drug screening experiments
conducted on cancer cell lines, with a focus on the discovery of biomarkers 
predictive of drug response. In this workshop, we focus specifically on the 
analysis of data that comes from combination experiments, where the response is 
measured as the viability of the cell line after treatment with a combination of 
compounds. 

The `r PGx` package is built on top of the `r CGx` package, which provides 
a set of common data structures and tools that are commonly useful in correlating  
phenotypic response to perturbation with compounds with the molecular state of 
samples. 

<!--  insert a comment here about the GXsuite -->

## CoreGx

### Package Nomenclature
<!-- Maybe make this into a sidebar comment? -->
...
used to share common data structure and methods across our suite of package.
...
# Overview of CoreGx Data Structures


Conceptually, in vitro pharmacogenomic experiments contain 3 main "dimensions": the samples assayed, 
the molecular features measured in those samples, and the compounds those samples were treated with. 
The `r CGx` and `r PGx` packages further extend the `SummarizedExperiment` and `MultiAssayExperiment` 
data structures to allow for the storage of the phenotypic readouts from the compound dimension, which 
often do not cleanly map onto a sample by feature matrix, together with the molecular data characterizing 
the treated samples. 
...
knitr::include_graphics("./CoreSet-class.drawio.png")
```

<!-- add an exercise here to get people familiar with the accessors of these packages -->
...
The `TRE` object shares much of its design with the `MultiAssayExperiment` and
`SummarizedExperiment`. We store dimension metadata in `rowData` and `colData`,
similar to an `SE` but also allow disjoint rows and columns to be stored in
`assays`, like an `MAE`. The major difference is that instead of using a
matrix or array for storing assay observations, we instead store them as a long
format `data.table` where integer keys define the relevant "shape" of the
underlying data and observations live within columns of that table.

For those familar with relational modelling from RDBMS systems,
our design has taken inspiration from the star schema often used in OLAP
(Online Analytical Processing) applications to improve the efficiency 
of analytic database queries.
...
The `TRE` differentiates between full assays and summary assays by the presence
of various identifier columns which can be viewed with the `idCols` method.
Each `TRE` has a set of `rowIDs` and `colIDs` which define these respective
dimensions. By storing an `assayIndex`, analagous to the `sampleMap` from the
`MAE` object, we are able to summarize over one or more row or column
identifiers while storing only the relevant rows within the assays slot, allowing
for a measure of compression relative to a matrix or array, where missing values
must be stored as `NA`.

The association between the various identifiers in the object are shown
below, with the `assayIndex` being the central repository for the graph like
relationships between the `rowIDs`, `colIDs` and each respective `assay`.

```{r tre_structure, echo=FALSE, fig.wide=TRUE, fig.cap="The structure of a `TreatmentResponseExperiment` centers around the `assayIndex` which contains a mapping from unique identifiers for row or columns and the primary key of each assay in the object. Summaries over rowKey and colKeys, and thus their corresponding identifiers, and made possible by repeating a givan assay key for ecah rowKey/colKey combination it has been summarized over."}
...
synergies between cancer therapies as a means of overcoming acquired treatment
resistance or overcoming in complete responses to first line therapies. To
accelerate the process of translating insights from _in vitro_ drug combination
experiments into clinical practice, we have extended the `CoreSet` and
`PharmacoSet` classes with a new object, the `TreatmentResponseExperiment`
(`TRE`), which has been specifically designed to handle high dimensional
biological stimilus-response data.
...
or patient tissue types. Additionally, we can imagine the object being useful
for other types of stimuli-response experiments across the biological sciences,
such as monitoring the response of nerve cells to various types of sensory
stimuli.
...
```{r nci_rawdata_echo, eval=TRUE, echo=TRUE}
...
For your own data in the wild, you will likely need to consult your experimental colleagues,
οr the original publication and explore the data to differentiate these two types of replicates
...
map cleaning to rows or columns gets assigned to "assay_metadata". The published
...
<!-- add section on subsetting and interacting with tre -->

## Working with a `TreatmentResponseExperiment`

### Subsetting and Extracting Data

One of the benefits of transforming your data into a `TRE` object is that you 
gain an expressive syntax with which to subset and extract data. Recall that when
we mapped the data onto the structure of the `TRE`, we defined row and column 
identifiers. For example, using the `rowIDs` method, we can see the identifiers 
that are associated with the rows of the object:

```{r tre_rowids}
(rowIDs(NCI_ALMANAC_tre)) 
```

These columns were combined to create pseudo-rownames for the object:

```{r tre_rownames}
tail(rownames(NCI_ALMANAC_tre)) 
```

These pseudo-rownames are concentrations of the values in the rowID columns, however,
they do not explicitly exist as rownames to any particular assay. While they can be 
used explicitly to get at specific observations, the power of the TRE comes when they 
are combined with wildcards, denoted by `*`, to extract data from the object. For example,
we could subset the TRE to a particular treatment as follows:

```{r tre_subset_oneobs}
NCI_ALMANAC_tre["Vorinostat:Vincristine:0.1:0.001:1",]
```

As you can see, this extracts the subset of data across all samples which were treated with 
the combination of Vorinostat and Vincristine at concentrations of 0.1 and 0.001, µm respectively.
If we want extract the response data for all the concentrations in this drug combo for a particular sample, we can 
then subset the object, and then extract the response assay as follows:

```{r tre_subset_combostar}
NCI_ALMANAC_tre["Vorinostat:Vincristine:*:*:*","A-498:*"] |>
  assay("response")
```

Here, we use the `*` character to denote that we want all the possible values for the last 
three rowID columns (treatment1dose, treatment2dose, and tech_rep), and the second colID
column (bio_rep).

In an example like this, it is possible to also use a shorthand, where the `*` is omitted 
for the trailing columns:
...
This is equivalent to the previous example, but is more concise. 

If you've noticed, there is one annoying detail here, which is that we need to know which of the 
two compounds is in the treatment1id column, and which is in the treatment2id column. Fortunately,
the `TRE` subset method provides us with two efficient ways around this. When we subset using only single
compound name, (for example `"Vincristine"`), it will automatically search all rowIDs for matches: 

```{r tre_subset_onecompound}
NCI_ALMANAC_tre["Vincristine", "A-498"]
```

We can string these subsets together to get a specific combination without knowing the ordering:

```{r tre_subset_onecompound_twice}
NCI_ALMANAC_tre["Vincristine", "A-498"]["Vorinostat", ]
```

Alternatively, the subset syntax supports or operations using the `|` character. This allows us to
express the same subset as follows:

```{r tre_subset_combo_or}
NCI_ALMANAC_tre["Vincristine:Vorinostat|Vorinostat:Vincristine", "A-498"]
```

Note that whether the assignment of a compound to treatment 1 or treatment 2 is arbitrary is dependent 
on the experimental design. In this case, the experiments were done in 3x3 or 5x5 designs, making the 
distinction meaningless. However, other experimental designs can use an 'anchor' compound approach, 
where one compound is tested at fewer (e.g. 2) concentrations together with a larger number of concentrations
for compound 2. In that case, the experiment "Vincristine:Vorinostat" could be different from "Vorinostat:Vincristine".

We have already seen that we can extract specific assays using the `assay` method. 
This is also possible with the `$` operatior. 

```{r subset_tre_assay_dollar}
NCI_ALMANAC_tre$response |>
  head()
```

Information about each specific 'row' or 'column' can be extracted using the `rowData` and `colData` methods:
...
Finally, if you notice above, a call to assay returns the data with the rowData and colData  
included in the data.frame. This is the default behaviour, however, it can be modified 
either to only include the row and column IDs by setting `metadata=FALSE`, or to not append
anything to the data.frame by setting `withDimnames=FALSE`. 

```{r subset_tre_assay_dollar_nometa}
NCI_ALMANAC_tre["Vorinostat:Vincristine", "A-498"] |>
  assay("response", metadata=FALSE)
```

```{r subset_tre_assay_dollar_nodims}
NCI_ALMANAC_tre["Vorinostat:Vincristine", "A-498"] |>
  assay("response", withDimnames = FALSE)
```

In this case, the internal row and column keys are returned instead of translating them back into 
the rowID and colID identifers that we defined earlier.

So far this has all been akin to syntactic sugar, giving us a more convenient way to run subsets
that would be possible to do on the long format data.table we read in above. The real advantages of the 
`TRE` subsetting syntax will be more evident as we begin to analyze the data and create summaries later in this workshop, 
where it can then keep the raw and derived datasets 'in sync' as the object is sliced using the row and column IDs.
...
### Hands on with a `TreatmentResponseExperiment`



To practice working with `TreatmentResponseExperiment` objects, we will
try to answer 3 questions:

1. How many drug combinations were tested involving Paclitaxel as one of the two compounds?
2. Which cell line was most sensitive (on average across concentrations) to co-treatment with 
Vincristine and Vorinostat?
3. Are there any biological replicates for drug combination experiments in the dataset?



## Combining with Omics Data into a `PharmacoSet`

We had already loaded the molecular data from the NCI-60 above,
and prior to this workshop we curated the cellids to match between
the datasets, so adding our molecular data to the NCI-ALMANAC `PharmacoSet`
requires no manual curation. We must simply subset the `NCI60` dataset
to those cell-lines present in NCI-ALMANAC.

Since we have our `MultiAssayExperiment` and `TreatmentResponseExperiment`
objects ready, we only need to extract and curate sample and treatment
...
```{r load_sample_metadata_files1, eval=TRUE}
...
```{r load_treatment_metadata_files1, eval=TRUE}
...
compatibility but will be deprecated in a future release.
...
comparing the observed combination response to what is expected from the response of cells to each drug separately. While
different models for "expected" combination effect will be discussed in the next section, we first review how monotherapy data are analyzed.

Monotherapy dose-response experiments are usually analyzed by first fitting a dose-response or dose-viability model to the observed measurements.
The most common model used in the literature is a form of sigmoid function, called the Hill Curve. The Hill Curve has its
basis in the Michaelis-Menten chemical kinetics model, while adding an extra parameter that is traditionally interpreted to account
for co-operativity of binding. To better match real observed data, the model can also be extended to include parameters
controlling the maximum and minimum observed viability.
...
$HS$, the Hill Slope, which is thought to be a measure of the cooperativity of binding. This parameter controls the steepness
...
```{r hillSlopeFig, fig.cap="Diagram of a logLogistic Hill Slope", echo=FALSE, warning=FALSE}
...
However, the `r BiocStyle::CRANpkg("drc")` CRAN package provides a larger variety of dose-response models, should it be appropriate in your application.
...
```{r sens_own_fit_first, eval=TRUE, warning=FALSE}
concentrations <- (1 / 2)^seq(0, 8) * 1
viabilities <- c(0.1, 5, 15, 23.8, 55.2, 85.9, 96.9, 98.4, 99.2)
pars <- logLogisticRegression(conc = concentrations, viability = viabilities)
...
As processsing monotherapy data is often a step in preparing for assessing synergy from combination experiments,
we will demonstrate how to use these functions together with a TreatmentResponseExperiment during our preprocessing
for synergy calculation.
...
response that each drug elicits in monotherapy. However, there are a few models widely applied in the literature, each with
...
However, some models, such as the ZIP, also perscribe a certain perspective or scoring method to assess synergy.

This vignette covers 4 different models for predicting what the effect of drug combination treatment should be.

_In the live workshop we focus only on the first 3 models._
...
As we introduce each method, we will go through the theoretical motivation behind it, the calculation, and then show how to compute
the expected viability/response for each model using the TreatmentResponseExperiment API.

### Preprocessing Monotherapy Data for Synergy Estimation
Prior to computing expected viabilities from combination experiments, the monotherapy data may need to be preprocessed. In our case,
the dataset we are using has replicate experiments for the monotherapy curves, as seen below:
```{r plotting_monotherapy_replicates}
...
The majority of these were replicates across plates, most probably so that monotherapy results could be plate matched to combination results.
Therefore, we can make two choices for how to handle these replicates, either average across plates, or to match analysis by plate whenever possible.
Ideally, we would recommend the latter approach, however, it is computationally more expensive (as we must fit monotherapy curves per-plate).
In interest of computation time, we average replicates across all experiments for our demonstration.

Here, we use the `endoaggregate` TRE method while setting the `by` argument to compute
an aggregation over each unique combination of `treatment1id`, `treatment1dose`, `sampleid` and
`bio_rep` (summarizing over technical replicates).
...
head(NCI_TRE$mono_response)

```


Additionally, computing the Loewe and ZIP scores requires knowing the Hill Fit parameters for the monotherapy experiments.
This is also a required step whenever it is necessary to interpolate between measured monotherapy doses to predict
for the measured combination viability doses.

We can fit the curves using `logLogisticRegression`, and save the monotherapy
fits back to the TreatmentResponseExperimentClassDiagram using the same endoaggregate pattern as above.
Since we are already calculating the Hill Curve fits for these experiments, we will also calculate the AAC and IC50 values.

Note that by default, `logLogisticRegression` returns the `E_inf` parameter on a percentage scale when the input data is
on a percentage scale. We convert to a fraction scale because that makes the subsequent calculations simpler.

```{r fitting_hill_curves_to_monotherapy, warning=FALSE}
  ...
  endoaggregate(
    {
      ...
      aac <- PharmacoGx::computeAUC(treatment1dose, Hill_fit = fit, area.type = "Fitted")
      list(
...
head(NCI_TRE$mono_profiles)


```

We will now extract the combination data into its own assay in the LongTable, and append the monotherapy Hill fits using the `mergeAssays` function, so that we can later use the fitted data
from above to calculate expected combination viability. We also filter our downstream analysis only to those experiments where the monotherapy fits were good (R squared better than 0.5).

Notice that we need to do two merges, one for appending the monotherapy Hill fits for the first compound treatment, and one for appending the monotherapy Hill fits for the second compound treatment.
Fortunately, the `suffixes` parameter will allow us to differentiate between the two once they are merged to the same table.

_The `mergeAssays` is built on top of R's merge function (actually, merge from `data.table`), and therefore accepts the same arguments to control how the merge is done._

```{r build_combo_profiles}
...
# Extract the combination response observations
NCI_TRE |>
  assay("response") |>
  subset(treatment2id != "") ->
  NCI_TRE$combo_response
...
NCI_TRE |>
  mergeAssays(
    ...
    y="mono_fits",
    by = c("treatment1id", "sampleid", "bio_rep"),
    sort = FALSE # Don't sort to avoid reordering and re-keying the data
  ) |>
  mergeAssays(
    x="combo_response",
    y="mono_fits",
...
Finally, we can make predictions for each of the measured combination doses using the corresponding fit parameters
and the Hill Curve models. This step is necessary when interpolating between monotherapy doses, but is often done
in any case, as the predicted viabilities can be less noisy than each individual monotherapy measurement.

```{r predicting_mono_response_for_combo_doses, warning=FALSE}
...
Last, we also put the measured combination viabilily onto a fractional, not percentage scale.

```{r fix_viability_percentage}

NCI_TRE$combo_response$viability <-
  NCI_TRE$combo_response$viability / 100

```

Finally, we can talk about the different models to compute the predicted response from monotherapy, and thereby compute estimates of
...
be increased. Practically, this case is not very often observed during _in vitro_ dose-response experiments, and the highest
single agent model is often a very weak baseline from which to compute synergy.


### Computing HSA Reference Viabilities

As one can imagine, this is very simple to do using the `min` or `max` functions in R.

```{r computeHSA, warning=FALSE}
...
with the fraction of cells alive after treatment with drug 2 in monotherapy.


```{r bliss_diagram, echo=FALSE, fig.cap="Diagram of Bliss Independence Assumption. Bliss independence assumes a constant fraction of cells is killed independently by each drug. In this diagram, the orange drug leaves 40% of the cells alive, and the blue drug leaves 60% of the cells alive. Therefore, Bliss predicts that the two drugs in combination will leave 24% of the cells alive."}
knitr::include_graphics("./BlissDiagram.png")
```
...
For convenience, `PharmacoGx::computeBliss(viability_1, viability_2)` is a more readable alias for computing the Bliss reference viability.
...
$$ 
\frac{x_1}{EC50_1 (\frac{1 - v_{loewe}}{v_{loewe} - E_{\infty,1}})^{\frac{1}{HS_1}}} + 
  \frac{x_2}{EC50_2 (\frac{1 - v_{loewe}}{v_{loewe} - E_{\infty,2}})^{\frac{1}{HS_2}}} = 1
$$

This formula can be solved numerically to find the value of $v_{loewe}$ for any two particular fitted curves.


While the Loewe Additivity model can be used to calculate the expected response at any pair of doses, more commonly, the Loewe Additivity is assessed in terms of the
Combination Index. The combination index measures the deviation from the Loewe assumption as a ratio at each point, by computing the Loewe model with the observed
combination viability instead of the reference viability. As it doesn't require calculating $v_{loewe}$, it is also easier to compute. The formula for the
Combination Index given a Loewe assumption is:

$$ 
CI_{loewe} = \frac{x_1}{EC50^1(\frac{v_c}{v_c - E_{\infty}^1})^{1/HS_1}} + 
  \frac{x_2}{EC50^2(\frac{v_c}{v_c - E_{\infty}^2})^{1/HS_2}} 
$$


```{r loewe_diagram, echo=FALSE, fig.cap="Diagram of Loewe Additivity Assumption. Loewe additivity assumes that when plotting viability as a function of both concentrations (bottom plots), lines of equal viability (isoboles) will be straight. Using this assumption, either the expected viability for each pair of concentrations can be calculated (left), or the combination index can be computed to assess synergy or antagonism directly (right)."}
knitr::include_graphics("./LoeweDiagram.png")
```
...
As mentioned above, the computation of the Loewe reference viability requires solving a non-linear implicitly defined equation. PharmacoGx provides
the `computeLoewe` function to do this calculation.


```{r computeLoewe, warning=FALSE}
# -- compute our drug synergy metrics
NCI_TRE |>
  endoaggregate(
...
Drug 2 present is that the presence of Drug 2 changes the baseline response, without affecting the potency of Drug 1. While the ZIP model can be extended to 3 and 4
parameter Hill Curve models, it is simplest to understand and explain using the 2 parameter Hill Curve, which is equivalent to the case where $E_{\infty}=0$.
...
$$
ZIP((v_1,x_1), (v_2,x_2)) = 
  (E_{\infty}^1 + \frac{1-E_{\infty}^1}{1+(\frac{x_1}{EC50^1})^{HS^1}})
  (E_{\infty}^2 + \frac{1-E_{\infty}^2}{1+(\frac{x_2}{EC50^2})^{HS^2}})
$$

While the formula for computing the ZIP reference value at each point works out to being the Bliss formula applied to the fitted curves, the ZIP assumptions
suggest a new method to assess syngergy or antagosism at each point. Building on the assumption that combination without synergy should not change the potency of
each particular drug, the ZIP $\delta$ score defines a measure comparing the difference between the fitted curve for each drug with and without the presence of the second
drug, taking the average of differences for the two drugs.

Mathematically, ZIP defines two values:
$$ 
v_c^{1 \leftarrow 2}((v_1,x_1), (v_2,x_2)) = 
  \frac{v_2}{1 + (\frac{x_1}{EC50^{1 \leftarrow 2}})^{HS^{1 \leftarrow 2}}}
$$
$$ 
v_c^{2 \leftarrow 1}((v_1,x_1), (v_2,x_2)) = 
  \frac{v_1}{1 + (\frac{x_2}{EC50^{2 \leftarrow 1}})^{HS^{2 \leftarrow 1}}}
$$

In this notation, {$1 \leftarrow 2$} means that we fit the Hill Curve for drug 1 after the addition of drug 2 at the dose $x_2$. Intuitively, we adjust for the observed effect
of adding $x_2$ of drug 2 ($v_2$), and then treat drug 1 as if it is a monotherapy experiment. The $\delta$ score is then defined as the difference of the viability predicted
by this curve and the ZIP predicted viability, averaged over treating each of the drugs as the "main drug":

$$
\delta((v_1,x_1), (v_2,x_2)) = 
  \frac{v_c^{1 \leftarrow 2} - v_{ZIP}}{2} + 
  \frac{v_c^{2 \leftarrow 1} - v_{ZIP}}{2}
$$

For more information on the ZIP model, we refer the reader to the original publication [@yadavSearchingDrugSynergy2015].
Alternatively, the `r BiocStyle::Biocpkg("synergyfinder")` package contains vignettes and the reference implementation for ZIP calculations.
...
doses is low (<6), the Bliss model is probably the reference of choice, even though it is known to be quite strict. When possible to calculate, the ZIP
...
the range is asymmetric around the reference.

In practice, our personal preference is to use the absolute deviation from the Bliss predicted value for downstream analysis.


_Note that we also skip the ZIP delta score computation in the interest of time._

```{r compute_combo_profiles, warning=FALSE}
...
combo_scores <- NCI_TRE$combo_scores
head(combo_scores)
```

Once we have a measure of synergy calculated for each dose combination in a
particular experiment, we often would like to visualize the whole surface of
drug-dose combinations to understand how synergy depends on dosage.

Below, we plot the Bliss score for an antagonistic combination of drugs,
Mitotane and Paclitaxel, on the A-549 cell line. If we recall the formula for
the Bliss score above, we took $v_{model} - v_c$, meaning that in this case,
negative numbers indicate antagonism, as there is more viability observed than
expected.

```{r bliss_score_plot, warning=FALSE}
library(ggplot2)
...
If you find yourself in a situation where synergy is observed only in a narrow dose range, the most extreme deviation from the reference (deviation from 0 or 1, depending on use of additive or fold-change metrics) may
...
```{r mean_bliss_calculation, warning=FALSE}
...
PharmacoGx makes an initial analysis for univariate biomarker discovery very simple, by providing the `drugSensitivitySig` function which will compute the association between molecular features and
drug response summaries (such as AAC) across the cell lines in a PharmacoSet.

There are two approaches implemented in PharmacoGx for measuring the association between molecular features and drug response. The first, older approach is to use a linear model to estimate the effect
of the molecular feature measurements on the drug response summary, and an analysis of variance F-test to test the significance of the molecular feature in predicting drug response over a Null model. If
multiple tissue types are present in the dataset, the Null model includes the tissue as an explanatory features, otherwise, it is just the intercept. This approach is most flexible, allowing for
both continous and binary measurements of drug response, as well as continous, categorical and binary molecular features. However, the estimate of the effect returned is a standardized regression coefficient,
which may be difficult to interpret and compare between biomarkers.
...
As our focus is on drug combination analysis in this workshop, we won't be demonstrating the usage of `drugSensitivitySig` to compute monotherapy associations with drug response. Rather, we skip directly
to computing associations with drug synergy. An example of monotherapy analysis can be found in the `r PGx` vignette: `vignette("PharmacoGx")`.
...
Above, we had already summarized our synergy surfaces into a single mean value for each experiment. Now, we need to reshape the "long" format table into a table of
drug combinations x cell lines.
```{r gene_sig_discovery, warning=FALSE, message=FALSE}
...
Finally, we can run our biomarker discovery. Note that we pass in the treatment x sample matrix to the `sProfiles` argument, short for sensitivity profiles.
For the purposes of this demonstration, we also subset the features we are computing over to 100 random genes, simply to save time.

```{r, echo=FALSE}
set.seed(42)
```

```{r biomarker_prediction, warning=FALSE}

gene_exp_sigs <- drugSensitivitySig(NCI_ALMANAC_2017,
  mDataType = "rnaseq.comp",
  sProfiles = synergy_summaries,
  modeling.method = "pearson",
  drugs="Vinblastine_+_Dasatinib",
  inference.method = "resampling",
  features = sample(rownames(molecularProfiles(NCI_ALMANAC_2017, "rnaseq.comp")), 100),
  nthread = 1
)
```

The object returned by `drugSensitivitySig` is a 3D array, with the first two dimensions of features X treatments, and the third dimension
returning computed statistics, the associated confidence intervals, p values, and FDR correction.

One intersting (toy) example we find here is that expression of a couple of genes are significantly correlated with antagonism
(negative Mean Bliss Score) for the combination of Vinblastine and Dasatinib.
...
To get a more global view of the results, we can also plot the resulting object, which by default will display as a volcano plot.
Note below that the object returned is a normal ggplot2 plot, and therefore we can add other annotations to it, such as the gene
labels. Below, we will add the labels for the two genes predicting antagonism between Vinblastine and Dasatinib to the plot.
...
We hope that this workshop leaves you feeling familiar with both the data structures defined in `r CGx` as well as the
analysis of drug combination experiments. Over time, our group will be curating public drug combination studies
and releasing `PharmacoSet` objects through our *[orcestra.ca](https://orcestra.ca)* platform, which will be available for everyone to download
and use in their own research. These studies will use identifiers mapped to the same databases (Pubchem and Cellosaurus)
as already existing monotherapy PSets, allowing studies combining monotherapy and combination response data.

Furthermore, on *[orcestra.ca](https://orcestra.ca)* we are curating clinical datasets combining genomic or transcriptomic profiling of cancer patient
samples with clinical drug response, mapped to the same identifiers and processed as much as possible with the same pipelines.
Much of the time, clinical treatments are done in combination therapies, as standard of care often involves more than 1 treatment.
As this resource grows, it will become possible to evaluate how well cell line combination experiment results translate to
clinical outcomes.
