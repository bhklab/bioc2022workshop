...
`CoreSet` to be used for other model systems
  * Drug (radiation in `RadioGx`) ahve become treatment, allowing the
...
difference being that the `SE` require symetric matrices (i.e., observations
...
easily ensure the reproducbility of the results as well as simplify their comparison
against other curated mono and combination therapy `PharmacoSet`s object.
In doing so, we help to improve the reproducbility and consistency of _in-vitro_
...
publication and explore the data to differential these two types of replicates
and assign identifiers which differentiate every unique observation within an
experiment.
...
experimental designs. However, data storage is not the point of the `TRE`.
...
within an assay. For example, we may want to take averages for all mono therapy
vaibilities over technical replicates. We can do so using the `aggregate` method
as follows:
...
the by argument which means that our summarize will be computed over it.

The resulting table will contain the identifer columns specified to `by`,
as well as any columsn which were summarized. By default, for
...
an aggregation over each unique combination of `treatment1id`, `sampleid` and
...
Additionally, computing the Loewe and ZIP scores requires knowing the Hill Fit parameters for the monotherapy experiemnts. 
...
We will now extract the combination data, and append the monotherapy Hill fits using the `merge` function, so that we can later use the fitted data from above to calculate expected combination viability.
Notice that we need to do two merges, one for appending the Hill fits for the first treatment, and one for appending the Hill fits for the second treatment. Fortunately, the `suffix` parameter
will allow us to differentiate between the two once they are merged to the same table.
...
treatmentResponse(nci) |>
  ...
  treatmentResponse(nci)$mono_fits

# Extract the combination response observations
treatmentResponse(nci) |>
  assay("response") |>
  subset(treatment2id != "") ->
  treatmentResponse(nci)$combo_response
...
treatmentResponse(nci) |>
  mergeAssays(
    ...
    y="mono_fits",
    by = c("treatment1id", "sampleid", "bio_rep"),
    sort = FALSE # workaround to not have weird keys as a result. 
  ) |>
  mergeAssays(
    x="combo_response",
    y="mono_fits",
...
and the Hill Curve models. This step is necessary when interpolating between monotherapy doses, but is often recommended
in either case, as the predicted viabilities can be less noisy than each individual monotherapy measurement. 
...
treatmentResponse(nci)$combo_response$viability <- treatmentResponse(nci)$combo_response$viability / 100
...
### Computing ZIP Reference Viabilities

Unlike Loewe, the ZIP reference viability is fairly easy to compute. In fact, it is simply the Bliss formula applied to the 2 paramater predicted viability. 

```{r computeZip, eval=FALSE}
ZIP <- {
...
ZIP also has a helpful function in PharmacoGx for its computation: `PharmacoGx::computeZIP(treatment1dose, treatment2dose, HS_1, HS_2, EC50_1, EC50_2)`
...
(treatmentResponse(nci) |>
  ...
  combo_scores <- treatmentResponse(nci)$combo_scores
```
...
This section is still under construction. Please check back after our
